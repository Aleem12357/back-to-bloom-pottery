      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2.5 sm:gap-3 group text-left focus:outline-none shrink-0 cursor-pointer"
          >
            <div className="relative">
              <img
                src="/assets/logo.jpg"
                alt="Back to Bloom Pottery Logo"
                className="w-11 h-11 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-clay-500 shadow-sm group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300 cursor-pointer"
              />
            ...
            <div>
              <div className="text-base sm:text-lg font-extrabold tracking-tight text-clay-950 group-hover:text-clay-600 transition-colors leading-snug cursor-pointer">
                <span>Back to Bloom</span>
              </div>
              <p className="text-[10px] sm:text-[11px] font-bold text-clay-600 tracking-wider uppercase cursor-pointer">
          ...
          <nav className="hidden xl:flex items-center gap-1.5 bg-white/90 px-4 py-1.5 rounded-full border border-clay-200/90 shadow-xs backdrop-blur-sm">
            {navLinks.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`group relative px-3.5 py-1.5 text-xs sm:text-sm font-semibold rounded-full transition-all duration-300 whitespace-nowrap cursor-pointer select-none ${
                    isActive
                ...
                >
                  <span className="relative z-10 flex items-center">
                    {link.label}
                    {link.badge && !isActive && (
                      <span className="ml-1.5 inline-block text-[9px] bg-terracotta/15 text-terracotta px-1.5 py-0.2 rounded-full font-bold uppercase tracking-wider">
                        {link.badge}
                      </span>
                    )}
                  </span>
                  {!isActive && (
                    <span className="absolute bottom-1 left-3.5 right-3.5 h-[2px] bg-gradient-to-r from-clay-500 to-terracotta rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-center" />
                  )}
                </button>
              );
            })}
          ...
          {/* Medium screen navigation fallback if xl is hidden */}
          <nav className="hidden lg:flex xl:hidden items-center gap-1 bg-white/90 px-3 py-1 rounded-full border border-clay-200/90 shadow-xs">
            {navLinks.map((link) => {
              const isActive = activeTab === link.id;
              return (
                <button
                  key={link.id}
                  onClick={() => handleNavClick(link.id)}
                  className={`group relative px-2.5 py-1 text-xs font-semibold rounded-full transition-all duration-300 whitespace-nowrap cursor-pointer ${
                    isActive
                      ? 'bg-clay-600 text-white shadow-xs font-bold'
                      : 'text-clay-800 hover:text-clay-950 hover:bg-clay-100/70'
                  }`}
                >
                  <span className="relative z-10">{link.label}</span>
                  {!isActive && (
                    <span className="absolute bottom-0.5 left-2.5 right-2.5 h-[2px] bg-gradient-to-r from-clay-500 to-terracotta rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-center" />
                  )}
                </button>
              );
            })}
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <a
              ...
              className="hidden sm:inline-flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold px-3.5 py-2 rounded-full shadow-xs hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
            ...
            <button
              ...
              className="hidden md:inline-flex items-center gap-1.5 bg-gradient-to-r from-clay-500 to-terracotta hover:from-clay-600 hover:to-terracotta/90 text-white text-xs sm:text-sm font-bold px-4 py-2 rounded-full shadow-xs hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
            >
              <Calendar size={15} />
